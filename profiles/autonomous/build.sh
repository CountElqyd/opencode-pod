#!/usr/bin/env bash
# Rebuilds autonomous.tar.gz from source files in src/
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SRC_DIR="$SCRIPT_DIR/src"
OUTPUT="$SCRIPT_DIR/autonomous.tar.gz"
...
echo "Building Autonomous profile v$VERSION..."
...
tar cf - --sort=name --owner=0 --group=0 --mtime="@0" \
  -C "$SRC_DIR" config/ skills/ agents/ commands/ \
