...
Ask these questions **ONE AT A TIME** via the question tool. Push on each one until the answer is specific, evidence-based, and uncomfortable. Comfort means the founder hasn't gone deep enough.
...
Ask these **ONE AT A TIME** via the question tool. The goal is to brainstorm and sharpen the idea, not interrogate.
...
- Ask via the question tool: "Should we build on this prior design or start fresh?"
...
**Privacy gate:** Before searching, use Use question tool: "I'd like to search for what the world thinks about this space to inform our discussion. This sends generalized category terms (not your specific idea) to a search provider. OK to proceed?"
...
Emit ONE question tool that lists every alternative (A/B and optionally C) as numbered options, using the preamble's Decision Brief Format section. The question tool call uses the tool, not prose — write the question text and call the tool.
...
If `$D serve` is not available or fails, fall back to Use question tool:
...
If Codex is available, use Use question tool:
...
Present the reviewed design doc to the user via the question tool:
...
Then use Use question tool: "Would you consider applying to Y Combinator?"
...
3. Use the question tool to offer opening the resources:
...
- **Questions ONE AT A TIME.** Never batch multiple questions into one question tool.
