...
1. **Collect symptoms:** Read the error messages, stack traces, and reproduction steps. If the user hasn't provided enough context, ask ONE question at a time via the question tool.
...
```bash
_FREEZE_SCRIPT="${GSTACK_SKILL_DIR}/../freeze/bin/check-freeze.sh"
[ -x "$_FREEZE_SCRIPT" ] || _FREEZE_SCRIPT="${GSTACK_SKILL_DIR}/../gstack-freeze/bin/check-freeze.sh"
...
3. **3-strike rule:** If 3 hypotheses fail, **STOP**. Use Use question tool:
   ```
...
- **If fix touches >5 files → question tool** about blast radius before proceeding.
- **Completion status:**
