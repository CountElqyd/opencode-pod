...
description: |
  ...
  Review which question prompts fire across gstack skills, set per-question preferences
...
2. question (use the contributor-specific framing only if `_CONTRIB=true`,
   ...
   **Contributor framing (only if `_CONTRIB=true`):**
   ...
   > question tool outcome locally to
...
1. Ask FIVE one-per-dimension declaration questions via individual
   question tool calls (one at a time). Use plain English, no jargon:
...
2. Confirm via the question tool:
...
2. For each unapplied proposal, present it as a numbered item and use
   question tool (one per call, per skill convention). Show:
...
3. If `NO_FREE_TEXT`: tell the user "No free-text answers since the last
   distill. Keep using gstack — `Other` responses on question tool feed
   this loop."
