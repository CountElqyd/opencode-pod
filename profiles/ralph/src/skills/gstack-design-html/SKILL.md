...
Then check for prior finalized.html. If `FINALIZED` was also found, use Use question tool:
...
3. If in plan-driven or freeform mode (no approved PNG), design from context:
   ...
   - **Freeform:** use question tool to gather what the user wants to build. Ask about:
...
If a framework is detected, use Use question tool:
...
LOOP:
  ...
  3. question tool (adjust wording based on mode):
...
Maximum 10 iterations. If the user hasn't said "done" after 10, use Use question tool:
