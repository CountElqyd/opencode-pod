...
feasibility blocker (not a preference), the question tool framing explicitly
...
Use question tool: you do, using the 6 principles, instead of the user.
...
Override: every question tool → auto-decide using the 6 principles.

**Override rules:**
- Mode selection: SELECTIVE EXPANSION
- Premises: accept reasonable ones (P6), challenge only clearly wrong ones
- **GATE: Present premises to user for confirmation** — this is the ONE question tool
  ...
  AGGREGATED_TASKS=$(jq -s \
    'group_by([.component, (.files | sort), .title])
     ...
     | if length == 0 then "_No actionable tasks emitted from any phase._" else
         map("- [ ] **\(.id) (\(.priority), human: \(.effort_human) / OC: \(.effort_cc)) — \(.component)** — \(.title)\n  - Surfaced by: \(.phase) — \(.source_finding)\n  - Files: \(.files | join(", "))") | join("\n")
...
Present as a message, then use Use question tool:
...
- **Two gates.** The non-auto-decided question tools are: (1) premise confirmation in Phase 1, and (2) User Challenges — when both models agree the user's stated direction should change. Everything else is auto-decided using the 6 principles.
