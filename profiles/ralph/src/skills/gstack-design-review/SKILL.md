...
If the output is non-empty (working tree is dirty), **STOP** and use Use question tool:
...
**If NO runtime detected** (no config files found): Use Use question tool:
...
$B url
```
If URL contains `/login`, `/signin`, `/auth`, or `/sso`: the site requires authentication. Use question tool: "This site requires authentication. Want to import cookies from your browser? Run `/setup-browser-cookies` first if needed."
...
11. **Clean working tree required.** If dirty, use the question tool to offer commit/stash/abort before proceeding.
