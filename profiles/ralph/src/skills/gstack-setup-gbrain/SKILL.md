...
dead Postgres URL). Fire a targeted question tool BEFORE Step 2:
...
`broken-config`, fire the same question tool again (the user picks again).
...
## Step 2: Pick a path (question tool)
...
Do NOT silently pick; fire the question tool.
...
Ask the D17 tier prompt via the question tool: "Which Supabase tier?" Present
...
List orgs, pick one (question tool if multiple):
...
resting state in OpenCode config (mode 0600).
...
~10ms. The token's resting state is OpenCode config (mode 0600 — Claude
...
- `deny` → do nothing.
- `unset` → Use question tool: "How should `<normalized-remote>` interact with
  gbrain?"
...
Separate Use question tool: "Also sync your gstack artifacts (CEO plans,
...
Otherwise (the "many transcripts on disk" path): question tool with
...
- Token: stored in OpenCode config (do not commit; never written to AGENTS.md)
...
in to git in many projects). It lives only in OpenCode config where `opencode mcp` placed it.
...
(local engines are inherently single-tenant). No question tool.
...
trust policy question via the question tool:
...
For each orphan, question tool per project: "Delete orphan project
