...
to guide deploy verification. If nothing is detected, ask the user via the question tool
...
Present the full dry-run results to the user via the question tool:
...
4. If no deploy workflows detected and no URL provided: use question tool once:
...
If deploy fails (`conclusion` is `failure`): use Use question tool:
...
If any fail: show the evidence (screenshot path, console errors, perf numbers). Use Use question tool:
