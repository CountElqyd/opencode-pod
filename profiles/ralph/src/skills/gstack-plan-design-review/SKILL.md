...
### 0D. Focus Areas
Use question tool: "I've rated this plan {N}/10 on design completeness. The biggest gaps are {X, Y, Z}. I'll generate visual mockups next, then review all 7 dimensions. Want me to focus on specific areas instead of all 7?"
...
path; use this for the question tool URL AND as the base for the reload
...
**PRIMARY WAIT: question tool with board URL**

After the board is serving, use question tool to wait for the user. Include the
...
**Do NOT use the question tool to ask which variant the user prefers.** The comparison
board IS the chooser. question tool is just the blocking wait mechanism.

**After the user responds to Use question tool:**
...
5. Reload the board in the user's browser (same tab) — the URL is per-board
   ...
   daemon.
6. The board auto-refreshes. **question tool again** with the same board URL to
   wait for the next round of feedback. Repeat until `feedback.json` appears.

**If `NO_FEEDBACK_FILE`:** The user typed their preferences directly in the
question tool response instead of using the board. Use their text response
as the feedback.

**POLLING FALLBACK:** Only use polling if `$D serve` fails (no port available).
In that case, show each variant inline using the read tool (so the user can see them),
then use Use question tool:
...
Use question tool to verify before proceeding.
...
**Do NOT use the question tool to ask which variant the user picked.** Read `feedback.json` — it already contains their preferred variant, ratings, comments, and overall feedback. Only use the question tool to confirm you understood the feedback correctly, never to re-ask what they chose.
...
5. question tool if there's a genuine design choice to resolve
...
**STOP.** question tool once per issue. Do NOT batch. Recommend + WHY. If no issues, say so and move on. Do NOT proceed until user responds.
...
**STOP.** question tool once per issue. Do NOT batch. Recommend + WHY.
...
Each decision = one question tool with recommendation + WHY + alternatives. Edit the plan with each decision as it's made.
...
Use question tool: "The review passes changed [list major design changes]. Want me to regenerate mockups to reflect the updated plan? This ensures the visual reference matches what we're actually building."
...
Follow the question tool format from the Preamble above. Additional rules for plan design reviews:
...
* **Zero findings:** if a section has zero findings, state "No issues, moving on" and proceed. Otherwise, use the question tool for each gap — a gap with an "obvious fix" is still a gap and still needs user approval before any change lands in the plan.
* **NEVER use the question tool to ask which variant the user prefers.** Always create a comparison board first (`$D compare --serve`) and open it in the browser. The board has rating controls, comments, remix/regenerate buttons, and structured feedback output. Use question tool ONLY to notify the user the board is open and wait for them to finish — not to present variants inline and ask "which do you prefer?" That is a degraded experience.
...
### TODOS.md updates
After all review passes are complete, present each potential TODO as its own individual question tool. Never batch TODOs — one per question. Never silently skip this step.
...
If any question tool goes unanswered, note it here. Never silently default to an option.
...
Use question tool to present the next step. Include only applicable options:
