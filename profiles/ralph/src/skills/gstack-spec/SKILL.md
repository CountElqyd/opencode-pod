...
- **1+ matches:** surface them to the user via the question tool: "Found {N} similar
...
Use question tool: A) edit, B) acknowledge and proceed, C) cancel. **On a PUBLIC repo,
...
block; MEDIUM PII/legal/internal confirm via the question tool; LOW surfaces). Full
...
2. **Exit 2 (MEDIUM)** — question tool per finding (cluster identical ids; PUBLIC
...
- **Score <7, iteration 2:** print "Quality gate: {score}/10 (after one
  revision). Codex still flags: {ambiguities}." Use question tool:
  - A) Ship anyway (file at this quality)
  - B) Save draft locally and stop (no issue filed)
  - C) One more revision attempt

Max 3 dispatches total. If still <7 after iter 3, question tool same options.
...
If `$DIRTY` is non-empty, Use question tool:
...
from the answer, re-prompt the question tool. The check must happen INSIDE
...
**D16 mandatory final-confirm gate:** Use question tool: "Spawn agent now? Last
...
- `T_PHASE1_START` — Phase 1 first question tool or first text emit
...
`question tool`. For open-ended interrogation, ask inline in the chat — the
