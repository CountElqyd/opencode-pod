...
if [ -d /workspace ]; then
  GUIDE_DST="/workspace/PROFILE-swarm.md"
  if [ ! -f "$GUIDE_DST" ] || [ "$INSTALLED" != "$VERSION" ]; then
    GUIDE_TMP="$(mktemp /workspace/.PROFILE-swarm.md.XXXXXX)"
    if tar xzOf "$TARBALL" guide/PROFILE-swarm.md > "$GUIDE_TMP" 2>/dev/null; then
